...
library(astropic)
knitr::kable(get_apod()) # no inputs will get today's image
```
...
```{r}
knitr::kable(get_apod(query  = list(start_date = "2018-04-01", end_date = "2018-04-03")))
```

### Count - `n` random images
```{r}
knitr::kable(get_apod(query = list(count = 5)))
```
