...
minmax_year <- words |> summarise(min = min(year), max = max(year))
...
word_counts |>
  ...
  labs(x = "", y = "",
       ...
       subtitle = glue::glue("{pull(minmax_year, min)}-{pull(minmax_year, max)}")) +
...
plot_dat |> 
  ...
  geom_line() +
  scale_x_continuous(breaks = seq(2007, 2025, 2), limits = c(2007, 2025)) +
  scale_y_continuous(labels = scales::percent) +
  labs(x = "Year", y = "Frequency of Word", 
       title = "Comparing Frequencies over the Years",
       subtitle = glue::glue("APOD {pull(minmax_year, min)}-{pull(minmax_year, max)}")
       ) +
  theme_classic(base_size = 14) 
```
