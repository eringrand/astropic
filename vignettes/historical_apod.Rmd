...
```{r words}
words <- hist_apod |>
  dplyr::select(date, title, explanation, media_type) |>
...
word_counts |>
  ggplot2::ggplot(aes(x = n, y = word, xend = 0, yend = word, color = word)) +
  ggplot2::geom_segment(size = 1.1, show.legend = FALSE) +
  ggplot2::geom_text(aes(label = n), hjust = -0.25, show.legend = FALSE) +
  ggplot2::labs(x = "", y = "",
  ...
  ggplot2::scale_x_continuous(limits = c(0, max(word_counts$n) + 400)) 
```
...
plot_dat |> 
  ggplot2::ggplot(aes(x = year, y = freq, color = word)) +
  ggplot2::geom_line() +
  ggplot2::scale_x_continuous(breaks = seq(2007, 2017, 2), limits = c(2007, 2018)) +
