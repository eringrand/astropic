...
The goal of `astropic` is to connect R to the [NASA APOD API](https://github.com/nasa/apod-api). The APOD API supports one image
at a time. In order to supply more than that, this package also includes the option of pulling by date ranges (of less than 2000 days at a time) and some historical data in tibble format.
...
Credit to the APOD API [contributors](https://github.com/nasa/apod-api/graphs/contributors) for all of their work in making the API, and the recent(ish) re-org.
...
```
## API Key
To start, you'll need a NASA API key. If you do not have one, you can get one [here](https://api.nasa.gov/index.html#apply-for-an-api-key). 
...
m31 <- get_apod(query = list(date = "2009-09-17"))  
...
## Contact
Come find me on BlueSky @[@astroeringrand.bsky.social](https://bsky.app/profile/astroeringrand.bsky.social)
